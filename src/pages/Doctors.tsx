...
export default function Doctors() {
  ...
  return <div className="space-y-6">
    ...
    <div className="stat-card"><div className="flex items-center gap-3 mb-4"><div className="relative flex-1 max-w-sm"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" /><Input placeholder="Search doctors..." className="pl-10 h-9" value={search} onChange={(e) => setSearch(e.target.value)} /></div></div>
      {isLoading ? <div className="space-y-3">{[1,2,3].map(i => <Skeleton key={i} className="h-12 w-full" />)}</div> : filtered.length === 0 ? <p className="text-center text-muted-foreground py-8">No doctors found.{canWriteMasterData ? ' Click "Add Doctor" to create one.' : ""}</p> : <table className="data-table"><thead><tr><th>Doctor</th><th>Specialty</th><th>Hospital</th><th>Contact</th>{canWriteMasterData && <th>Actions</th>}</tr></thead><tbody>{filtered.map((d: any) => <tr key={d.id} className="hover:bg-muted/50 transition-colors"><td className="font-medium flex items-center gap-2"><Stethoscope className="w-4 h-4 text-primary" />{d.doctor_name}</td><td><Badge variant="secondary">{d.specialty || "—"}</Badge></td><td className="text-muted-foreground">{d.hospital || "—"}</td><td className="text-muted-foreground">{d.contact || "—"}</td>{canWriteMasterData && <td><div className="flex items-center gap-1"><button onClick={() => openEdit(d)} className="p-1.5 rounded hover:bg-muted" aria-label="Edit doctor"><Pencil className="w-4 h-4 text-muted-foreground" /></button><button onClick={() => handleDelete(d.id)} className="p-1.5 rounded hover:bg-destructive/10" aria-label="Delete doctor"><Trash2 className="w-4 h-4 text-destructive" /></button></div></td>}</tr>)}</tbody></table>}
    </div>
    {canWriteMasterData && <><EntityDialog open={dialogOpen} onOpenChange={setDialogOpen} title={editingDoctor ? "Edit Doctor" : "Add Doctor"}><form onSubmit={handleSubmit} className="space-y-4"><div><Label>Doctor Name *</Label><Input value={form.doctor_name} onChange={(e) => setForm({ ...form, doctor_name: e.target.value })} required className="mt-1" /></div><div><Label>Specialty</Label><Input value={form.specialty} onChange={(e) => setForm({ ...form, specialty: e.target.value })} className="mt-1" /></div><div><Label>Hospital</Label><Input value={form.hospital} onChange={(e) => setForm({ ...form, hospital: e.target.value })} className="mt-1" /></div><div><Label>Contact</Label><Input value={form.contact} onChange={(e) => setForm({ ...form, contact: e.target.value })} className="mt-1" /></div><Button type="submit" className="w-full" disabled={insertMutation.isPending || updateMutation.isPending}>{editingDoctor ? "Update Doctor" : "Add Doctor"}</Button></form></EntityDialog><BulkImportDialog open={importOpen} onOpenChange={setImportOpen} title="Import Doctors" columns={[{ key: "doctor_name", label: "Doctor Name", required: true },{ key: "specialty", label: "Specialty" },{ key: "hospital", label: "Hospital" },{ key: "contact", label: "Contact" }]} onImport={async (rows) => { if (!canWriteMasterData) return denyWrite(); await bulkInsert.mutateAsync(rows); }} /></>}
  </div>;
}
