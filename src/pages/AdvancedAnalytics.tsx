  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-xl border bg-card p-4 space-y-4" aria-label="Analytics filters">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
        ...
        <label className="text-xs space-y-1"><span className="text-muted-foreground">Claim status</span><select value={status} onChange={e => setStatus(e.target.value)} className="w-full rounded-md border bg-background px-2 py-2"><option value="all">All statuses</option>{[...new Set((claims || []).map((c: any) => String(c.status || "").toLowerCase()).filter(Boolean))].sort().map(s => <option key={s} value={s}>{s.replace(/_/g, " ")}</option>)}</select></label>
