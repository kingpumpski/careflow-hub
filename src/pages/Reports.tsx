...
export default function Reports() {
  ...
  const reportData = useMemo(() => {
    ...
    return (insurers || []).map((ins: any) => {
      ...
      const outstanding = submitted - paid - tax;

      return { company: ins.company_name, submitted, paid, tax, outstanding, color: ins.color };
    }).filter(r => r.submitted > 0 || r.paid > 0);
  ...
  const handleExportPDF = () => {
    const data = reportData.map(r => [r.company, `GH¢ ${r.submitted.toLocaleString()}`, `GH¢ ${r.paid.toLocaleString()}`, `GH¢ ${r.tax.toLocaleString()}`, `GH¢ ${r.outstanding.toLocaleString()}`]);
    data.push(["Grand Total", `GH¢ ${grandSubmitted.toLocaleString()}`, `GH¢ ${grandPaid.toLocaleString()}`, `GH¢ ${grandTax.toLocaleString()}`, `GH¢ ${grandOutstanding.toLocaleString()}`]);
    exportReportPDF(`${activePeriod} Account Balance Report`, data, ["Insurance Company", "Submitted", "Paid", "WHT", "Outstanding"], companyInfo);
  };
  ...
  const handleExportExcel = () => {
    ...
    rows.push({ "Insurance Company": "Grand Total", "Submitted (GH¢)": grandSubmitted, "Paid (GH¢)": grandPaid, "WHT (GH¢)": grandTax, "Outstanding (GH¢)": grandOutstanding });
          ...
          <tbody>
            {reportData.map((r) => (
              <tr key={r.company} className="hover:bg-muted/50 transition-colors">
                ...
                <td className="text-warning">{r.tax.toLocaleString()}</td>
                <td className="text-destructive font-semibold">{r.outstanding.toLocaleString()}</td>
              </tr>
            ))}
            {reportData.length === 0 && (
              <tr><td colSpan={5} className="text-center text-muted-foreground py-8">No data for selected period</td></tr>
            )}
          </tbody>
          {reportData.length > 0 && (
            <tfoot>
