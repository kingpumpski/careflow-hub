...
export default function ClaimsSettlement() {
  ...
  const createPeriod = async () => {
    ...
    setBusy(true);
    try {
      await insertPeriod.mutateAsync({ facility_id: facilityId, insurance_company_id: form.insurerId, period_start: form.periodStart, period_end: form.periodEnd, period_type: form.periodType, total_claims_submitted: submitted, withholding_tax_rate: rate, settlement_status: "awaiting_payment" });
      setForm(emptyForm);
  ...
  const recordAdvice = async (row: any) => {
    ...
    try {
      ...
      await updatePeriod.mutateAsync({ id: row.id, payment_received: payment, rejection_amount: rejection, actual_withholding_tax: actual, payment_advice_reference: form.paymentAdviceReference.trim(), payment_advice_date: form.paymentAdviceDate, settlement_status: "payment_advice_received" });
  ...
  const reconcilePeriod = async (row: any) => {
    if (!canReconcileSettlement({ settlementStatus: row.settlement_status, paymentReceived: row.payment_received, rejectionAmount: row.rejection_amount, actualWithholdingTax: row.actual_withholding_tax, paymentAdviceReference: row.payment_advice_reference, paymentAdviceDate: row.payment_advice_date })) {
      toast({ title: "Reconciliation not ready", description: "Payment advice must be completely recorded before the period can be reconciled.", variant: "destructive" });
      return;
    }
    setBusy(true);
    try {
      await updatePeriod.mutateAsync({ id: row.id, settlement_status: "reconciled", confirmed_at: new Date().toISOString() });
      toast({ title: "Settlement reconciled", description: "The period is now marked as reconciled." });
