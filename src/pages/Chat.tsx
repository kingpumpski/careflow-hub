...
export default function Chat() {
  ...
  const otherUsers = useMemo(
    () => typedProfiles.filter((p) => p.id !== user?.id),
    [typedProfiles, user],
  );
  ...
  const loadMessages = async () => {
    ...
    let query = supabase
      .from("chat_messages")
      .select("id,sender_id,recipient_id,subject,body,is_broadcast,thread_key,read_by,created_at")
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="stat-card p-8 text-center text-sm text-muted-foreground">
          {permissionsLoading ? "Checking access…" : "Sign in to use Internal Chat."}
        </div>
      </div>
    );
  }
        ...
        <div className="stat-card !p-0 flex flex-col overflow-hidden">
          <div className="p-3 border-b border-border">
            <div className="relative">
              ...
              <Input placeholder="Search users..." className="pl-10 h-9" value={search} onChange={(e) => setSearch(e.target.value)} maxLength={100} />
            </div>
          </div>
        ...
        <div className="stat-card !p-0 flex flex-col overflow-hidden">
          ...
          <div className="p-3 border-t border-border space-y-2 bg-card">
            ...
            <div className="flex gap-2">
              ...
              />
              <Button onClick={() => void send()} disabled={sending || !body.trim()} className="gap-2 self-end">
                <Send className="w-4 h-4" />Send
              </Button>
            </div>
            <p className="text-[10px] text-muted-foreground">Ctrl/Cmd + Enter to send · {body.length}/{MAX_BODY_LENGTH}</p>
          </div>
        </div>
      </div>
    </div>
