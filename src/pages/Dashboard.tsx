...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3"><div><h1 className="page-title">CareFlow Hub</h1><p className="page-description">Executive claims intelligence — revenue, risk and operational health in real time.</p></div><div className="flex items-center gap-2"><Button variant="outline" size="sm" onClick={() => navigate("/schedule")}>Generate schedule</Button><Button size="sm" onClick={() => navigate("/reports")}>Reports</Button></div></div>
      ...
      <ChartCard title="AI Insight Panel" subtitle="Claims intelligence, revenue forecast and settlement prediction" action={<div className="flex items-center gap-2"><AIInsightBadge /><Button size="sm" variant="outline" onClick={generateNarrative} disabled={aiLoading}>{aiLoading ? <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5 mr-1.5" />}{aiLoading ? "Analysing" : "Ask AI"}</Button></div>}>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">{insights.map((insight) => <InsightCard key={insight.id} insight={insight} />)}</div>
        {aiNarrative && <div className="mt-4 rounded-xl border border-primary/25 bg-primary/5 p-4"><p className="text-xs font-semibold text-primary mb-2 flex items-center gap-1.5"><Sparkles className="w-3.5 h-3.5" /> AI management recommendations</p><p className="text-sm whitespace-pre-wrap leading-relaxed text-foreground/90">{aiNarrative}</p></div>}
      </ChartCard>
