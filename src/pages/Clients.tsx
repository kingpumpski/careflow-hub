...
export default function Clients() {
  ...
  const openNew = () => { if (!canWriteMasterData) return denyWrite(); setEditing(null); setForm({ company_name: "", insurance_company_id: "", contact_person: "", email: "", phone: "" }); setDialogOpen(true); };
  const openEdit = (c: any) => { if (!canWriteMasterData) return denyWrite(); setEditing(c); setForm({ company_name: c.company_name, insurance_company_id: c.insurance_company_id || "", contact_person: c.contact_person || "", email: c.email || "", phone: c.phone || "" }); setDialogOpen(true); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) { await updateMutation.mutateAsync({ id: editing.id, ...payload }); toast({ title: "Client updated" }); }
      else { await insertMutation.mutateAsync(payload); toast({ title: "Client added" }); }
      setDialogOpen(false);
  ...
  return (
    <div className="space-y-6">
      <div className="page-header flex items-start justify-between">
        <div><h1 className="page-title">Client Companies</h1><p className="page-description">Manage registered client companies and their insurance mappings</p></div>
        {canWriteMasterData && <div className="flex gap-2"><Button variant="outline" onClick={() => setImportOpen(true)} className="gap-2"><Upload className="w-4 h-4" />Import</Button><Button onClick={openNew} className="gap-2"><Plus className="w-4 h-4" />Add Client</Button></div>}
      </div>
      ...
      {canWriteMasterData && <>
        <EntityDialog open={dialogOpen} onOpenChange={setDialogOpen} title={editing ? "Edit Client" : "Add Client"}><form onSubmit={handleSubmit} className="space-y-4"><div><Label>Company Name *</Label><Input value={form.company_name} onChange={(e) => setForm({ ...form, company_name: e.target.value })} required className="mt-1" /></div><div><Label>Insurance Partner</Label><select className="mt-1 w-full h-9 rounded-md border border-input bg-background px-3 text-sm" value={form.insurance_company_id} onChange={(e) => setForm({ ...form, insurance_company_id: e.target.value })}><option value="">Select insurer...</option>{(insurers || []).map((i: any) => <option key={i.id} value={i.id}>{i.company_name}</option>)}</select></div><div><Label>Contact Person</Label><Input value={form.contact_person} onChange={(e) => setForm({ ...form, contact_person: e.target.value })} className="mt-1" /></div><div><Label>Email</Label><Input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="mt-1" /></div><div><Label>Phone</Label><Input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="mt-1" /></div><Button type="submit" className="w-full" disabled={insertMutation.isPending || updateMutation.isPending}>{editing ? "Update Client" : "Add Client"}</Button></form></EntityDialog>
        <BulkImportDialog open={importOpen} onOpenChange={setImportOpen} title="Import Client Companies" columns={[{ key: "company_name", label: "Company Name", required: true },{ key: "contact_person", label: "Contact Person" },{ key: "email", label: "Email" },{ key: "phone", label: "Phone" }]} onImport={async (rows) => { if (!canWriteMasterData) return denyWrite(); await bulkInsert.mutateAsync(rows); }} />
      </>}
    </div>
  );
}
