...
export default function UsersPage() {
  ...
  const UserActions = ({ user }: { user: ManagedUser }) => (
    <div className="flex flex-wrap items-center gap-1">
      <Button variant="ghost" size="sm" onClick={() => openAccess(user)} className="gap-1 min-h-9 flex-1 sm:flex-none"><Settings2 className="w-4 h-4" />Access</Button>
      <button type="button" title="Send reset email" aria-label={`Send reset email to ${user.full_name || user.email || "user"}`} onClick={() => resetPassword(user)} className="min-h-9 min-w-9 p-1.5 rounded hover:bg-muted inline-flex items-center justify-center"><KeyRound className="w-4 h-4 text-muted-foreground" /></button>
      <button type="button" title="Set new password" aria-label={`Set new password for ${user.full_name || user.email || "user"}`} onClick={() => { setPwForm({ user_id: user.id, new_password: "" }); setPwOpen(true); }} className="min-h-9 min-w-9 p-1.5 rounded hover:bg-muted inline-flex items-center justify-center"><Lock className="w-4 h-4 text-muted-foreground" /></button>
      <button type="button" title="Delete user" aria-label={`Delete ${user.full_name || user.email || "user"}`} onClick={() => deleteUser(user)} className="min-h-9 min-w-9 p-1.5 rounded hover:bg-destructive/10 inline-flex items-center justify-center"><Trash2 className="w-4 h-4 text-destructive" /></button>
    </div>
  );

  return <div className="space-y-4 sm:space-y-6 min-w-0">
    <div className="page-header flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><h1 className="page-title">User Management</h1><p className="page-description">Create accounts and allocate roles and granular privileges.</p></div><Button onClick={() => setInviteOpen(true)} className="gap-2 w-full sm:w-auto shrink-0"><UserPlus className="w-4 h-4" />Add User</Button></div>
    <div className="stat-card p-3 sm:p-5"><div className="flex items-center gap-3 mb-4"><div className="relative w-full sm:max-w-sm"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" /><Input placeholder="Search users..." className="pl-10 h-9" value={search} onChange={(e) => setSearch(e.target.value)} /></div></div>
      {loading ? <div className="space-y-3">{[1,2,3].map((i) => <Skeleton key={i} className="h-12 w-full" />)}</div> : filtered.length === 0 ? <p className="text-center text-muted-foreground py-8">No users found.</p> : <>
        <div className="hidden md:block overflow-x-auto -mx-3 sm:-mx-5"><table className="data-table min-w-[760px]"><thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Status</th><th>Actions</th></tr></thead><tbody>{filtered.map((p) => { const role = roleMap.get(p.id) ?? "viewer"; return <tr key={p.id} className="hover:bg-muted/50 transition-colors"><td className="font-medium"><span className="inline-flex items-center gap-2 min-w-0"><Shield className="w-4 h-4 shrink-0 text-muted-foreground" /><span className="truncate max-w-[180px]">{p.full_name || "Unknown"}</span></span></td><td className="text-muted-foreground"><span className="block max-w-[220px] truncate" title={p.email || undefined}>{p.email || "—"}</span></td><td><Badge variant="outline" className={roleColors[role] || ""}>{ROLE_LABELS[role] || role}</Badge></td><td><Badge variant="outline">{p.email_confirmed_at ? "Active" : "Unverified"}</Badge></td><td><UserActions user={p} /></td></tr>; })}</tbody></table></div>
        <div className="md:hidden space-y-3">{filtered.map((p) => { const role = roleMap.get(p.id) ?? "viewer"; return <article key={p.id} className="rounded-lg border border-border bg-background/50 p-3 sm:p-4 min-w-0"><div className="flex items-start gap-3 min-w-0"><div className="h-9 w-9 rounded-lg bg-muted flex items-center justify-center shrink-0"><Shield className="w-4 h-4 text-muted-foreground" /></div><div className="min-w-0 flex-1"><p className="font-semibold text-sm break-words">{p.full_name || "Unknown"}</p><p className="text-xs text-muted-foreground break-all mt-0.5">{p.email || "—"}</p></div><Badge variant="outline" className={`shrink-0 ${roleColors[role] || ""}`}>{ROLE_LABELS[role] || role}</Badge></div><div className="mt-3 flex items-center justify-between gap-3 border-t border-border/60 pt-3"><Badge variant="outline" className="shrink-0">{p.email_confirmed_at ? "Active" : "Unverified"}</Badge><UserActions user={p} /></div></article>; })}</div>
      </>}
    </div>
    <EntityDialog open={accessOpen} onOpenChange={setAccessOpen} title="Role & Privileges"><form onSubmit={saveAccess} className="space-y-5 max-h-[75vh] overflow-y-auto pr-1"><p className="text-sm text-muted-foreground">Configure <strong>{selectedUser?.full_name || selectedUser?.email}</strong>. Overrides are saved separately from the role defaults.</p><div><Label>Role</Label><select className="mt-1 w-full h-9 rounded-md border border-input bg-background px-3 text-sm" value={selectedRole} onChange={(e) => { const role = e.target.value as AppRole; setSelectedRole(role); const next = { ...selectedAccess }; permissions.forEach((p) => { next[p.key] = ROLE_PERMISSIONS[role].includes(p.key); }); setSelectedAccess(next); }}>{Object.entries(ROLE_LABELS).map(([k,v]) => <option key={k} value={k}>{v}</option>)}</select></div>{categories.map((category) => <div key={category} className="rounded-lg border p-3 space-y-2"><h3 className="font-medium text-sm">{category}</h3>{permissions.filter((p) => p.category === category).map((p) => <label key={p.key} className="flex items-start gap-3 rounded-md p-2 hover:bg-muted/50 cursor-pointer"><input type="checkbox" className="mt-1" checked={Boolean(selectedAccess[p.key])} onChange={(e) => setSelectedAccess((prev) => ({ ...prev, [p.key]: e.target.checked }))} /><span className="min-w-0"><span className="block text-sm font-medium break-words">{p.label}</span><span className="block text-xs text-muted-foreground mt-0.5 break-words">{p.description}</span></span></label>)}</div>)}<Button type="submit" className="w-full" disabled={savingAccess}>{savingAccess ? "Saving access..." : "Save Role & Privileges"}</Button></form></EntityDialog>
