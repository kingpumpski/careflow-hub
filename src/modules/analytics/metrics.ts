...
export { resolveAggregateStatus, resolveOutstandingStatus, latestPeriodKeys, resolvePeriodKey };
export function computeExecutiveKpis(claims:any[]=[],payments:any[]=[],withholdingTax:any[]=[]):ExecutiveKpis{
 const rejected=claims.filter(c=>String(c.status||"").toLowerCase()==="rejected"), active=claims.filter(c=>String(c.status||"").toLowerCase()!=="rejected"), netSubmitted=sum(active,"claim_amount"), rejectedAmount=sum(rejected,"claim_amount"), grossSubmitted=netSubmitted+rejectedAmount, paymentsReceived=sum(payments,"amount_paid"), tax=sum(withholdingTax,"tax_amount");
...
export function rowYear(row:any):number|null{const explicit=Number(row?.claim_year??row?.year);if(Number.isFinite(explicit)&&explicit>0)return explicit;const key=resolvePeriodKey(row);return key?Number(key.slice(0,4)):null;}
export function rowMonth(row:any):number|null{const key=resolvePeriodKey(row);return key?Number(key.slice(5,7)):null;}
...
export function buildTrendSeries(claims:any[]=[],payments:any[]=[],withholdingTax:any[]=[]):TrendPoint[]{
 const keys=latestPeriodKeys([claims,payments,withholdingTax],12);
 let cumulative=0;
 return keys.map(key=>{
  const [year,month]=key.split("-").map(Number),monthClaims=claims.filter(c=>resolvePeriodKey(c)===key),submitted=sum(monthClaims.filter(c=>String(c.status||"").toLowerCase()!=="rejected"),"claim_amount"),rejected=sum(monthClaims.filter(c=>String(c.status||"").toLowerCase()==="rejected"),"claim_amount"),paid=sum(payments.filter(p=>resolvePeriodKey(p)===key),"amount_paid"),tax=sum(withholdingTax.filter(t=>resolvePeriodKey(t)===key),"tax_amount");
