...
export default function SettlementExceptionRegister({ periods }: { periods: any[] }) {
  ...
  return <section className="stat-card space-y-4">
    ...
    {isLoading ? <p className="text-sm text-muted-foreground">Checking settlement exceptions…</p> : !active.length ? <div className="rounded-md border p-5 text-sm text-muted-foreground">No open settlement exceptions detected.</div> : <div className="space-y-3">{active.map((row: any) => <div key={row.id} className="rounded-md border p-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div className="space-y-1"><div className="flex flex-wrap items-center gap-2"><Badge variant={row.severity === "critical" ? "destructive" : row.severity === "warning" ? "outline" : "secondary"}>{row.severity}</Badge><Badge variant="outline">{String(row.status).replaceAll("_", " ")}</Badge><span className="text-sm font-medium">{row.title}</span></div><p className="text-sm text-muted-foreground">{row.description}</p><p className="text-xs text-muted-foreground">Detected {new Date(row.detectedAt).toLocaleString()}</p></div><div className="flex flex-wrap gap-2">{row.status === "open" && <Button size="sm" variant="outline" disabled={busyId === row.id} onClick={() => void changeStatus(row.id, "under_review")}><ClipboardCheck className="mr-1 h-4 w-4" />Review</Button>}{(row.status === "open" || row.status === "under_review") && <><Button size="sm" disabled={busyId === row.id} onClick={() => void changeStatus(row.id, "resolved")}><CheckCircle2 className="mr-1 h-4 w-4" />Resolve</Button><Button size="sm" variant="ghost" disabled={busyId === row.id} onClick={() => void changeStatus(row.id, "waived")}>Waive</Button></>}</div></div></div>)}</div>}
  </section>;
}
