...
export default function SettlementManagementReport({ periods }: { periods: Row[] }) {
  ...
  const outstanding = summary.totalSubmitted - summary.totalPaymentReceived - summary.totalRejectionAmount - summary.totalActualWht;
  ...
  return <section className="stat-card space-y-4">
    <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
      <div><h2 className="font-semibold">Management reconciliation report</h2><p className="text-sm text-muted-foreground">Operational summary of settlement completeness. Figures come only from recorded settlement periods and payment advice.</p></div>
      <Badge variant="secondary">OFFLINE OPERATIONAL DATA</Badge>
    </div>
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
      ...
      <div className="rounded-md border p-3"><div className="flex items-center gap-2 text-sm text-muted-foreground"><AlertTriangle className="h-4 w-4" />Overdue</div><div className="mt-1 text-2xl font-semibold">{summary.overdueCount}</div></div>
    ...
    <div className="grid gap-4 md:grid-cols-4 text-sm">
      ...
      <div><span className="text-muted-foreground">Recorded residual*</span><div className="font-medium">{money(outstanding)}</div></div>
    </div>
    <p className="text-xs text-muted-foreground">*Residual is a management arithmetic indicator only; it is not an inferred receivable, claim balance, or payment obligation.</p>
