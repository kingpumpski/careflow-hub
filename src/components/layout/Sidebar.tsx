...
export const NAV_GROUPS: NavGroup[] = [
  ...
  { label: "Claims Intelligence", icon: Shield, children: [
  ...
  { label: "Pre-Authorization", icon: FileCheck, children: [
    { label: "Authorization Studio", path: "/pre-auth/studio", permission: "preauth.read" },
  ...
  { label: "Administration", icon: Settings2, children: [
...
export default function Sidebar({ onNavigate }: { onNavigate?: () => void } = {}) {
  ...
  return <aside className="w-64 max-w-[88vw] h-screen sticky top-0 bg-sidebar flex flex-col border-r border-sidebar-border no-print overflow-hidden"><div className="p-4 sm:p-5 border-b border-sidebar-border shrink-0"><div className="flex items-center gap-3 min-w-0"><div className="w-9 h-9 rounded-xl bg-sidebar-primary flex items-center justify-center shrink-0"><Activity className="w-5 h-5 text-sidebar-primary-foreground" /></div><div className="min-w-0"><h1 className="font-heading text-base font-bold text-sidebar-primary-foreground truncate">CareFlow Hub</h1><p className="text-[11px] text-sidebar-muted truncate">{roleLabel ?? "Revenue Operations"}</p></div></div></div><nav className="flex-1 min-h-0 p-2.5 sm:p-3 space-y-0.5 overflow-y-auto overscroll-contain">{NAV_GROUPS.map((group) => { if (!group.children) return <Link key={group.path} to={group.path!} onClick={onNavigate} className={`sidebar-item min-h-10 ${isActive(group.path!) ? "sidebar-item-active" : ""}`}><group.icon className="w-[18px] h-[18px] shrink-0" />{group.label}</Link>; const children = group.children.filter(visible); if (!children.length) return null; const open = expanded.includes(group.label); return <div key={group.label}><button onClick={() => toggle(group.label)} aria-expanded={open} className="sidebar-item w-full min-h-10 justify-between"><span className="flex items-center gap-3 min-w-0"><group.icon className="w-[18px] h-[18px] shrink-0" /><span className="truncate">{group.label}</span></span>{open ? <ChevronDown className="w-4 h-4 shrink-0" /> : <ChevronRight className="w-4 h-4 shrink-0" />}</button>{open && <div className="ml-3 sm:ml-4 mt-0.5 space-y-0.5 border-l border-sidebar-border pl-2 sm:pl-3">{children.map((child) => <Link key={child.path} to={child.path} onClick={onNavigate} className={`sidebar-item min-h-10 text-[13px] min-w-0 ${isActive(child.path) ? "sidebar-item-active" : ""}`}><span className="truncate">{child.label}</span></Link>)}</div>}</div>; })}</nav><div className="p-2.5 sm:p-3 border-t border-sidebar-border shrink-0"><button onClick={signOut} className="sidebar-item w-full min-h-10 text-destructive/80 hover:text-destructive"><LogOut className="w-[18px] h-[18px] shrink-0" />Sign Out</button></div></aside>;
}
