...
export default function PreAuthStudio({ onBack, editData }: Props) {
  ...
  const [saveSuggestion, setSaveSuggestion] = useState(true);
  const [busy, setBusy] = useState(false);

  const provider = useMemo(() => ({
    name: settings?.find?.((s: any) => s.key === "provider_name")?.value || "",
    address: settings?.find?.((s: any) => s.key === "provider_address")?.value || "",
    phone: settings?.find?.((s: any) => s.key === "provider_phone")?.value || "",
  }), [settings]);
  ...
  useEffect(() => {
    ...
    setExtraDiagnoses(Array.isArray(editData.custom_diagnoses) ? editData.custom_diagnoses : []); setTemplateId(editData.template_id || "");
    void (async () => { const { data } = await (supabase.from("preauth_items") as any).select("id,description,quantity,unit_price").eq("preauth_id", editData.id).order("id"); if (data?.length) setItems(data.map((x: any) => ({ id: String(x.id), description: x.description || "", quantity: Number(x.quantity) || 1, unitPrice: Number(x.unit_price) || 0 }))); })();
  }, [editData]);

  useEffect(() => {
    const q = clientSearch.trim();
    if (!q) { setSuggestions([]); return; }
    const timer = window.setTimeout(() => { void searchClientSuggestions(q).then(setSuggestions).catch(() => setSuggestions([])); }, 180);
    return () => window.clearTimeout(timer);
  }, [clientSearch]);

  const total = useMemo(() => items.reduce((sum, x) => sum + Math.max(0, Number(x.quantity) || 0) * Math.max(0, Number(x.unitPrice) || 0), 0), [items]);
  const updateClient = (patch: Partial<typeof client>) => setClient((x) => ({ ...x, ...patch }));
  const updateInsurance = (patch: Partial<typeof insurance>) => setInsurance((x) => ({ ...x, ...patch }));
  const selectSuggestion = (x: ClientSuggestion) => { updateClient({ name: x.client_name, dob: x.date_of_birth || "", phone: x.phone || "", email: x.email || "", address: x.address || "", identifier: x.identifier || "", membership: x.membership_number || "" }); setClientSearch(""); setShowSuggestions(false); };
  const selectInsurer = (id: string) => { const x = (insurers || []).find((i: any) => i.id === id); if (x) updateInsurance({ id, name: x.company_name || "", phone: x.phone || "", email: x.email || "" }); };
  const selectProcedure = (id: string) => { setProcedureId(id); const x = (procedures || []).find((p: any) => p.id === id); if (x) setItems([{ ...makeItem(), description: x.procedure_name || "", unitPrice: Number(x.default_tariff) || 0 }]); };
  const selectTemplate = (id: string) => { setTemplateId(id); const t = (templates || []).find((x: any) => x.id === id); if (!t) return; try { const raw = typeof t.items === "string" ? JSON.parse(t.items) : t.items; if (Array.isArray(raw) && raw.length) setItems(raw.map((x: any) => ({ ...makeItem(), description: x.description || "", quantity: Number(x.quantity || 1), unitPrice: Number(x.unitCharge ?? x.unit_price ?? 0) }))); } catch { toast({ title: "Template could not be loaded", variant: "destructive" }); } if (t.procedure_id) setProcedureId(t.procedure_id); if (t.diagnosis_code_id) setDiagnosisId(t.diagnosis_code_id); };
  const addItem = () => setItems((x) => [makeItem(), ...x]);
  const removeItem = (id: string) => setItems((x) => x.length === 1 ? x : x.filter((i) => i.id !== id));
  const patchItem = (id: string, patch: Partial<Item>) => setItems((x) => x.map((i) => i.id === id ? { ...i, ...patch } : i));
  const addCustomDiagnosis = () => { const v = customDiagnosis.trim(); if (v && !extraDiagnoses.includes(v)) setExtraDiagnoses((x) => [...x, v]); setCustomDiagnosis(""); };

  const submit = async () => {
    setBusy(true);
    try {
      if (!client.name.trim()) throw new Error("Client name is required.");
      if (!insurance.name.trim()) throw new Error("Insurer name is required. You can type it manually even when it is not in the insurer directory.");
      const valid = items.filter((x) => x.description.trim());
      if (!valid.length) throw new Error("Add at least one service or charge line.");
      if (valid.some((x) => Number(x.quantity) <= 0 || Number(x.unitPrice) < 0)) throw new Error("Quantities must be greater than zero and charges cannot be negative.");
      const diagnosis = diagnosisId ? (() => { const d = (diagnoses || []).find((x: any) => x.id === diagnosisId); return d ? `${d.code} - ${d.description}` : null; })() : null;
      const payload = {
        patient_id: client.patientId || null, client_name: client.name.trim(), client_date_of_birth: client.dob || null,
        client_phone: client.phone || null, client_email: client.email || null, client_address: client.address || null,
        client_identifier: client.identifier || null, client_membership_number: client.membership || null,
        insurance_company_id: insurance.id || null, insurer_name: insurance.name.trim(), insurer_member_number: insurance.memberNumber || null,
        insurer_plan_name: insurance.plan || null, insurer_policy_reference: insurance.policy || null, insurer_phone: insurance.phone || null, insurer_email: insurance.email || null,
        doctor_id: doctorId || null, procedure_id: procedureId || null, diagnosis, procedure_date: procedureDate || null, total_cost: total,
        provider_name: provider.name || null, provider_address: provider.address || null, provider_phone: provider.phone || null,
        status: editing ? editData.status : "pending", accommodation_days: accommodationDays === "" ? null : Number(accommodationDays),
