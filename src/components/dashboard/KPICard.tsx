...
export interface KPIFrame { label: string; value: string; hint?: string; progress?: number; }
export interface KPICardProps {
  title: string; value: string; hint?: string; description?: string;
  trend?: "up" | "down" | "flat"; tone?: "primary" | "success" | "warning" | "destructive" | "accent" | "info";
  icon: LucideIcon; progress?: number; onClick?: () => void; frames?: KPIFrame[]; interval?: number; frameIndex?: number; frameCount?: number;
}
const toneRing: Record<NonNullable<KPICardProps["tone"]>, string> = {
  ...
  destructive: "bg-destructive/10 text-destructive", accent: "bg-accent/10 text-accent", info: "bg-info/10 text-info",
};
const toneBar: Record<NonNullable<KPICardProps["tone"]>, string> = {
  primary: "bg-primary", success: "bg-success", warning: "bg-warning", destructive: "bg-destructive", accent: "bg-accent", info: "bg-info",
};
...
export default function KPICard({ title, value, hint, description, trend = "flat", tone = "primary", icon: Icon, progress, onClick, frames, interval = 6000, frameIndex, frameCount }: KPICardProps) {
  ...
  const slides: KPIFrame[] = [{ label: "All time", value, hint, progress }, ...(frames || [])];
  ...
  useEffect(() => { if (localIndex >= slides.length) setLocalIndex(0); }, [slides.length, localIndex]);
  ...
  const handleInfoKeyDown = (event: React.KeyboardEvent<HTMLSpanElement>) => {
    if (event.key === "Enter" || event.key === " ") { event.preventDefault(); event.stopPropagation(); setInfoOpen((open) => !open); }
  };
  return (
    <div role={onClick ? "button" : undefined} tabIndex={onClick ? 0 : undefined} aria-describedby={descriptionId}
      ...
      className={cn("kpi-card group relative isolate text-left w-full min-w-0 overflow-hidden", onClick && "cursor-pointer hover:-translate-y-0.5 hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-offset-2")}
    >
      ...
      <div role="tooltip" aria-hidden={!infoOpen} className={cn("absolute z-50 inset-2 max-w-[calc(100%-1rem)] max-h-[calc(100%-1rem)] rounded-lg border border-border bg-popover/98 p-3 text-xs leading-relaxed text-popover-foreground shadow-lg backdrop-blur-sm overflow-y-auto overflow-x-hidden overscroll-contain transition-all duration-150", infoOpen ? "opacity-100 translate-y-0 visible" : "opacity-0 translate-y-1 invisible pointer-events-none")}>
        <div className="flex items-start gap-2 min-w-0 max-w-full"><Info className="w-3.5 h-3.5 shrink-0 mt-0.5 text-primary" aria-hidden="true" /><p className="min-w-0 max-w-full break-words [overflow-wrap:anywhere] whitespace-normal">{resolvedDescription}</p></div>
      </div>
    </div>
  );
}
