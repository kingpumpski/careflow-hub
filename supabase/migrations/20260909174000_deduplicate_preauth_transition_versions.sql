create unique index if not exists preauth_versions_preauth_version_uidx
  on public.preauth_versions(preauth_id, version_number);
...
begin
  ...
  v_current := case lower(coalesce(v_row.current_state,v_row.status,''))
    when 'draft' then 'Draft' when 'pending' then 'PendingApproval' when 'pendingapproval' then 'PendingApproval'
    when 'approved' then 'Approved' when 'rejected' then 'Rejected' when 'completed' then 'Completed' else 'Draft' end;
  ...
  set current_state=v_target,status=v_status,
