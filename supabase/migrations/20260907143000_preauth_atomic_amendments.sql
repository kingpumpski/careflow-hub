-- Atomic amendment path. Existing versions remain immutable; the deferred
-- version trigger captures the final post-update state as a new version.
...
BEGIN
  ...
  SELECT EXISTS (
    SELECT 1 FROM public.preauthorization_versions v WHERE v.preauth_id = p_preauth_id
  ) INTO v_version_exists;

  -- Ensure a legacy record has an immutable baseline before its first amendment.
  IF NOT v_version_exists THEN
    PERFORM public.create_preauth_version(p_preauth_id, 'baseline');
  END IF;
  ...
  v_fingerprint := encode(digest(convert_to(
    ...
    coalesce((SELECT string_agg(coalesce(trim(x->>'description'),'') || ':' || ((x->>'quantity')::NUMERIC)::TEXT || ':' || ((x->>'unit_price')::NUMERIC)::TEXT, '|' ORDER BY ord)
              FROM jsonb_array_elements(p_items) WITH ORDINALITY AS a(x, ord)), ''), 'UTF8'), 'sha256'), 'hex');

  PERFORM pg_advisory_xact_lock(hashtextextended('preauth-create:' || v_fingerprint, 0));
  IF EXISTS (SELECT 1 FROM public.pre_authorizations p WHERE p.id <> p_preauth_id AND p.dedup_fingerprint = v_fingerprint AND p.status NOT IN ('cancelled','void')) THEN
  ...
  LOOP
    ...
    VALUES (p_preauth_id, trim(v_item->>'description'), (v_item->>'quantity')::NUMERIC,
            (v_item->>'unit_price')::NUMERIC, round((v_item->>'quantity')::NUMERIC * (v_item->>'unit_price')::NUMERIC, 2));
  END LOOP;
