-- Serialize finalization attempts for the same facility + duplicate signature.
-- The browser review is advisory; this server boundary is authoritative so two
-- concurrent operators cannot both freeze matching requests.
...
DECLARE
  ...
  v_duplicate public.pre_authorizations%rowtype;
...
BEGIN
  ...
  -- Idempotency is checked before duplicate rejection so a retried request
  -- always receives the same already-created result.
  ...
  -- pg_advisory_xact_lock is transaction-scoped and therefore releases even
  -- when the transaction rolls back. This lock is deliberately keyed by both
  -- facility and signature so identical requests in different facilities do
  -- not block each other.
  IF nullif(trim(coalesce(v_row.duplicate_signature,'')),'') IS NOT NULL THEN
    PERFORM pg_advisory_xact_lock(hashtextextended(v_row.facility_id::text || ':' || v_row.duplicate_signature, 0));
    SELECT * INTO v_duplicate
      FROM public.pre_authorizations
     WHERE facility_id=v_row.facility_id
       AND duplicate_signature=v_row.duplicate_signature
       AND id<>v_row.id
       AND status IN ('draft','prepared')
     ORDER BY created_at ASC
     LIMIT 1;
    IF FOUND THEN
      RAISE EXCEPTION 'DUPLICATE_PREAUTH' USING errcode='23505',
        DETAIL = coalesce(v_duplicate.request_number, v_duplicate.id::text);
    END IF;
  END IF;
...
COMMENT ON FUNCTION public.finalize_preauthorization_handoff(uuid,jsonb,numeric,jsonb,jsonb,text,text,text) IS 'Atomically freezes a pre-authorization document revision, rejects facility-scoped duplicate finalization, and records an idempotent email handoff package.';
