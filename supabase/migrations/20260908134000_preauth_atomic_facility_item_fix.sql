-- Keep the atomic pre-authorization writer compatible with the tightened
-- preauth_items tenant boundary introduced immediately before this migration.
...
BEGIN
  ...
  IF round(coalesce((p_payload->>'total_cost')::numeric,0),2) <> v_total THEN RAISE EXCEPTION 'TOTAL_COST_MISMATCH'; END IF;
  v_fp := public.preauth_complete_fingerprint(p_payload,p_items);

  SELECT id INTO v_existing FROM public.pre_authorizations
  WHERE facility_id=v_facility AND dedup_fingerprint=v_fp
  ...
  INSERT INTO public.pre_authorizations (
    ...
    doctor_id,procedure_id,diagnosis,procedure_date,insurance_company_id,insurer_name,insurer_member_number,insurer_plan_name,insurer_phone,insurer_email,insurer_policy_reference,
    provider_name,provider_address,provider_phone,total_cost,status,created_by,accommodation_days,clinical_notes,approval_notes,custom_diagnoses,diagnosis_ids,template_id,dedup_fingerprint,duplicate_checked_at
  ) VALUES (
    v_facility,NULLIF(p_payload->>'patient_id','')::uuid,trim(p_payload->>'client_name'),NULLIF(p_payload->>'client_date_of_birth','')::date,NULLIF(p_payload->>'client_phone',''),NULLIF(p_payload->>'client_email',''),NULLIF(p_payload->>'client_address',''),NULLIF(p_payload->>'client_identifier',''),NULLIF(p_payload->>'client_membership_number',''),
    ...
    NULLIF(p_payload->>'provider_name',''),NULLIF(p_payload->>'provider_address',''),NULLIF(p_payload->>'provider_phone',''),v_total,coalesce(NULLIF(p_payload->>'status',''),'pending'),(select auth.uid()),NULLIF(p_payload->>'accommodation_days','')::integer,NULLIF(p_payload->>'clinical_notes',''),NULLIF(p_payload->>'approval_notes',''),coalesce(p_payload->'custom_diagnoses','[]'::jsonb),coalesce(p_payload->'diagnosis_ids','[]'::jsonb),NULLIF(p_payload->>'template_id','')::uuid,v_fp,now()
  ) RETURNING * INTO v_row;

  FOR v_item IN SELECT value FROM jsonb_array_elements(p_items) LOOP
    INSERT INTO public.preauth_items(facility_id,preauth_id,description,quantity,unit_price,amount)
    VALUES(v_facility,v_row.id,trim(v_item->>'description'),(v_item->>'quantity')::numeric,(v_item->>'unit_price')::numeric,round((v_item->>'quantity')::numeric*(v_item->>'unit_price')::numeric,2));
  END LOOP;
