alter table public.document_ingest_batches
...
alter table public.document_ingest_records
  drop constraint if exists document_ingest_records_status_check;

alter table public.document_ingest_records
  add column if not exists changed_fields jsonb not null default '{}'::jsonb,
  add column if not exists conflict_fields jsonb not null default '{}'::jsonb;

alter table public.document_ingest_records
  add constraint document_ingest_records_status_check
...
create index if not exists insurance_companies_document_ingest_lookup_idx
  on public.insurance_companies (lower(regexp_replace(trim(company_name), '\\s+', ' ', 'g')));
    ...
    if v_entity = 'insurance_company' then
      ...
      if v_match_id is null then
        ...
        if jsonb_object_length(v_conflict_fields) > 0 then
          ...
          set patient_name = coalesce(patient_name, v_patient_name),
              procedure_name = coalesce(procedure_name, v_procedure_name),
              preauth_id = coalesce(preauth_id, v_pre_auth_id)
          where id = v_match_id;
          v_updated := v_updated + 1;
          insert into public.document_ingest_records (batch_id, entity, record_fingerprint, source_record, status, matched_record_id, changed_fields)
          values (v_batch_id, v_entity, v_fingerprint, v_record, 'updated', v_match_id, v_changed_fields);
