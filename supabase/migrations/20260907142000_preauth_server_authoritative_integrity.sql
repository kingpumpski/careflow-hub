...
CREATE OR REPLACE FUNCTION public.create_preauthorization_atomic(
  p_payload JSONB,
  p_items JSONB,
  p_save_client_suggestion BOOLEAN DEFAULT false
)
...
BEGIN
  ...
  LOOP
    ...
    IF v_description IS NULL OR length(v_description) > 500 THEN
      RAISE EXCEPTION 'INVALID_ITEM_DESCRIPTION:%', v_index;
    END IF;
    BEGIN
      v_quantity := (v_item->>'quantity')::NUMERIC;
      v_unit_price := (v_item->>'unit_price')::NUMERIC;
    EXCEPTION WHEN invalid_text_representation OR numeric_value_out_of_range THEN
      RAISE EXCEPTION 'INVALID_ITEM_NUMBER:%', v_index;
    END;
    IF v_quantity IS NULL OR v_quantity <= 0 OR v_quantity > 1000000 THEN
      RAISE EXCEPTION 'INVALID_ITEM_QUANTITY:%', v_index;
    END IF;
    IF v_unit_price IS NULL OR v_unit_price < 0 OR v_unit_price > 100000000000 THEN
      RAISE EXCEPTION 'INVALID_ITEM_UNIT_PRICE:%', v_index;
    END IF;
  ...
  IF p_payload ? 'total_cost' AND NULLIF(trim(p_payload->>'total_cost'), '') IS NOT NULL THEN
    ...
    EXCEPTION WHEN invalid_text_representation OR numeric_value_out_of_range THEN
      RAISE EXCEPTION 'INVALID_TOTAL_COST'; END;
  ...
  v_fingerprint := encode(digest(convert_to(
    ...
    coalesce(p_payload->>'procedure_date', '') || '|' ||
    coalesce(p_payload->>'procedure_id', '') || '|' ||
    coalesce(p_payload->>'doctor_id', '') || '|' ||
    coalesce(p_payload->>'diagnosis', '') || '|' || v_total::TEXT || '|' ||
    coalesce((SELECT string_agg(
      coalesce(trim(x->>'description'), '') || ':' ||
      ((x->>'quantity')::NUMERIC)::TEXT || ':' ||
      ((x->>'unit_price')::NUMERIC)::TEXT,
      '|' ORDER BY ord
    ) FROM jsonb_array_elements(p_items) WITH ORDINALITY AS a(x, ord)), ''), 'UTF8'
  ), 'sha256'), 'hex');

  PERFORM pg_advisory_xact_lock(hashtextextended('preauth-create:' || v_fingerprint, 0));

  IF EXISTS (SELECT 1 FROM public.pre_authorizations p
             WHERE p.dedup_fingerprint = v_fingerprint
               AND p.status NOT IN ('cancelled', 'void')) THEN
    RAISE EXCEPTION 'DUPLICATE_PREAUTH';
  END IF;

  INSERT INTO public.pre_authorizations (
    patient_id, doctor_id, procedure_id, diagnosis, procedure_date,
    insurance_company_id, provider_name, provider_address, provider_phone,
    total_cost, status, created_by, client_name, client_date_of_birth,
    client_phone, client_email, client_address, client_identifier,
    client_membership_number, insurer_name, insurer_member_number,
    ...
    v_total, coalesce(NULLIF(p_payload->>'status',''), 'draft'), (select auth.uid()),
    v_client_name, NULLIF(p_payload->>'client_date_of_birth','')::DATE,
  ...
  LOOP
    ...
    VALUES (v_row.id, trim(v_item->>'description'), (v_item->>'quantity')::NUMERIC,
  ...
  IF p_save_client_suggestion THEN
    INSERT INTO public.preauth_client_suggestions (
      ...
      identifier, membership_number, source_patient_id, use_count, last_used_at,
      created_by, updated_at
    ) VALUES (
      public.preauth_normalize_identity(v_client_name), v_client_name,
    ...
    DO UPDATE SET client_name = excluded.client_name, date_of_birth = excluded.date_of_birth,
      ...
      source_patient_id = excluded.source_patient_id,
...
CREATE OR REPLACE FUNCTION public.find_duplicate_preauthorization_text_first(
  p_client_name TEXT, p_client_date_of_birth DATE DEFAULT NULL,
  p_insurer_name TEXT DEFAULT NULL, p_insurer_member_number TEXT DEFAULT NULL,
  p_procedure_date DATE DEFAULT NULL, p_procedure_id UUID DEFAULT NULL,
  p_doctor_id UUID DEFAULT NULL, p_diagnosis TEXT DEFAULT NULL,
  p_total_cost NUMERIC DEFAULT NULL, p_exclude_id UUID DEFAULT NULL
)
RETURNS TABLE(id UUID, request_number TEXT, status TEXT, created_at TIMESTAMPTZ,
              client_name TEXT, insurer_name TEXT)
LANGUAGE sql SECURITY DEFINER SET search_path = ''
AS $$
  SELECT p.id, p.request_number, p.status, p.created_at, p.client_name, p.insurer_name
  FROM public.pre_authorizations p
  WHERE p.id IS DISTINCT FROM p_exclude_id
